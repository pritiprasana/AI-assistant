...
Commands:
- ingest: Index a codebase into the vector store
- ask: Ask a question with optional RAG context
...
def ingest(
    ...
    if store.count() > 0 and not force:
        console.print(
            f"[yellow]Vector store already has {store.count()} documents.[/yellow]"
        )
        console.print("[yellow]Use --force to re-index.[/yellow]")
        return

    # Clear old data on force re-index to avoid stale duplicates
    ...
    console.print(
        f"[bold green]Indexing complete! Total chunks: {store.count()}[/bold green]"
    )
...
def ask(
    ...
    if use_rag:
        ...
        if store.count() == 0:
            console.print(
                "[yellow]No documents indexed. Run 'slot-assistant ingest' first.[/yellow]"
            )
            use_rag = False
        else:
            results = store.query(question, n_results=10)
            if results:
                # Build context with source info
                parts = []
                for r in results:
                    meta = r["metadata"]
                    header = meta.get("filename", "unknown")
                    if meta.get("name") and meta["name"] != "anonymous":
                        header += f"::{meta['name']}"
                    parts.append(f"[{header}]\n{r['content']}")

                context = "\n\n---\n\n".join(parts)
                console.print(
                    f"[green]Retrieved {len(results)} code chunks[/green]\n"
                )
...
def serve(
    host: str = typer.Option(
        os.getenv("API_HOST", "0.0.0.0"), help="Host to bind to"
    ),
    port: int = typer.Option(
        int(os.getenv("API_PORT", "8000")), help="Port to bind to"
    ),
):
    ...
    backend = detect_llm_backend()
    console.print(
        f"[bold blue]Starting API server with {backend.upper()} backend[/bold blue]"
    )
    ...
    if backend == "ollama":
        ...
        if not check_ollama_available():
            console.print(
                "[bold yellow]Warning: Ollama server not detected![/bold yellow]"
            )
            console.print("[yellow]Make sure Ollama is installed and running:[/yellow]")
            console.print(
                "[yellow]  1. Download from https://ollama.ai/download[/yellow]"
            )
